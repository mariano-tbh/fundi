...
export type Render = ({ ref, signal }: { ref: ShadowRoot, signal: AbortSignal }) => void

export type Def<Props extends {} = {}> = (props: Props) => Render

function autotag(name: string, prefix: string = $component.PREFIX) {
    return prefix + '-' + (name.match(/[A-Z][a-z]+/g)?.join('-').toLowerCase() || `component-${nanoid()}`)
}
...
export function $component<Props extends {} = {}>(
    ...
    options: { tag?: string, shadowRoot?: ShadowRootInit, extends?: keyof HTMLElementTagNameMap, prefix?: string } = {}
) {
    const { prefix, tag = autotag(definition.name, prefix), shadowRoot = { mode: 'open' }, extends: _extends } = options
    ...
    return function instance(props: Props): Directive<Node> {
